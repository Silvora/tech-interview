...
vue2使用ssr的痛点
- 单例模式问题：在 Vue2 SSR 中，每次用户请求都需要创建一个新的 Vue 实例，这会导致状态污染风险和管理复杂性。
- 客户端激活(hydration)复杂性：需要确保服务端渲染的 DOM 结构与客户端生成的完全匹配，否则会导致激活失败。
- 数据预取机制：
  - 需要区分首屏数据获取(使用 asyncData)和组件切换数据获取

  - 首屏数据需要在服务端渲染前完成

  - 组件切换数据通常通过 mixin 在 beforeMount 钩子中处理

  - 这种分离的机制增加了代码复杂性
- 全局状态管理：需要特别注意避免共享状态在多个请求间污染。

- 构建配置复杂：需要为客户端和服务端分别配置 webpack 构建。

Vue3 对 SSR 的改进
- 更简单的 API：提供了 createSSRApp() 和更简洁的 SSR 渲染 API。

- 改进的 hydration：

  - 更智能的 hydration 机制，对不匹配的情况更宽容

  - 支持部分 hydration，提高性能

- 更优雅的数据预取：

  - 使用 setup() 函数和 Composition API 可以更自然地处理异步数据

  - 不再需要 asyncData 和 mixin 的分离机制

  - 支持在 setup() 中使用 async/await

- 更好的 TypeScript 支持：整个 SSR 流程都有更好的类型推断。

- Vite 集成：Vue3 与 Vite 深度集成，简化了 SSR 的构建配置。

- 更高效的渲染：Vue3 的编译器优化也提升了 SSR 的性能。

- Pinia 状态管理：Vue3 推荐的状态管理库 Pinia 对 SSR 更友好。
